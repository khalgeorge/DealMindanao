...
<header class="admin-header">
  <div>
    <h1 class="text-xl font-black text-gray-900">Edit Brand</h1>
    <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">{{ $brand->name }}</p>
  </div>
...
<div class="admin-content max-w-2xl">
  ...
  <form method="POST" action="{{ route('admin.brands.update', $brand) }}" class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    @csrf
    @method('PUT')
    ...
    <div class="p-8 space-y-6">
      ...
      <div>
        ...
        <textarea name="description" rows="3"
          ...
          class="input w-full resize-none @error('description') border-red-300 bg-red-50 @enderror">{{ old('description', $brand->description) }}</textarea>
        @error('description')
        <p class="text-[11px] text-red-500 font-bold mt-1">{{ $message }}</p>
        @enderror
      </div>
