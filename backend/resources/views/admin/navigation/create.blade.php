...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500">Create a new navigation link.</p>
    </div>
...
<div class="space-y-6">
    ...
    <form action="/admin/navigation" method="POST" class="card max-w-3xl">
        @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input name="location" class="input" value="{{ old('location', 'header') }}" required>
            @error('location')
                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
