...
<script>
  ...
  function loadCart() {
    ...
    // Consolidate truly duplicate items (same product + same variant)
    const consolidatedCart = [];
    cart.forEach(item => {
      const key = item.cart_key ?? String(item.id);
      const existing = consolidatedCart.find(c => (c.cart_key ?? String(c.id)) === key);
      if (existing) {
            ...
            <div>
              <div class="flex justify-between items-start gap-2">
                <div>
                  ...
                  ${item.variant ? `<span class="inline-block text-[10px] font-bold text-brand-700 bg-brand-50 border border-brand-200 px-2 py-0.5 rounded-full mb-1">${item.variant}</span>` : ''}
                </div>
