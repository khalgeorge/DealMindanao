{{--
    Facebook Customer Chat Plugin — public pages only
    ─────────────────────────────────────────────────
    • Uses Meta's official Customer Chat Plugin (embeds chat inline on the page).
    • Only rendered when APP_ENV=production AND FACEBOOK_PAGE_ID is set.
    • IMPORTANT: Your domain must be whitelisted in Facebook Page Settings →
      Messenger → Customer Chat Plugin → Whitelisted Domains.
--}}
@php
    $fbPageId = config('services.facebook.page_id', '');
@endphp

@if(app()->environment('production') && $fbPageId)
{{-- Facebook SDK for Customer Chat Plugin --}}
<div id="fb-root"></div>
<script>
    window.fbAsyncInit = function () {
        FB.init({
            xfbml: true,
            version: 'v22.0'
        });
    };
    (function (d, s, id) {
        var js, fjs = d.getElementsByTagName(s)[0];
        if (d.getElementById(id)) return;
        js = d.createElement(s);
        js.id = id;
        js.src = 'https://connect.facebook.net/en_US/sdk/xfbml.customerchat.js';
        fjs.parentNode.insertBefore(js, fjs);
    }(document, 'script', 'facebook-jssdk'));
</script>

{{-- Customer Chat Widget — opens inline on the page, no redirect --}}
<div class="fb-customerchat"
     attribution="setup_tool"
     page_id="{{ $fbPageId }}"
     greeting_dialog_display="hide"
     greeting_dialog_delay="5"
     logged_in_greeting="Hi! How can we help you today? 😊"
     logged_out_greeting="Hi! Chat with us on Messenger.">
</div>
@endif

